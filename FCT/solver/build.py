...
def build_optimizer(cfg: CfgNode, model: torch.nn.Module) -> torch.optim.Optimizer:
    ...
    for module in model.modules():
        for key, value in module.named_parameters(): #recurse=False):
            if cfg.MODEL.RPN.FREEZE_RPN and "proposal_generator" in key:
    ...
    print(keys_list)
    if cfg.SOLVER.SOLVER_TYPE == "sgd":
        optimizer = torch.optim.SGD(
            params, cfg.SOLVER.BASE_LR, momentum=cfg.SOLVER.MOMENTUM, nesterov=cfg.SOLVER.NESTEROV
        )
        print("============ using the SGD optimizer for all the parameters ============")
        optimizer = maybe_add_gradient_clipping(cfg, optimizer)
    elif cfg.SOLVER.SOLVER_TYPE == "adamw":
        optimizer = torch.optim.AdamW(
            params, cfg.SOLVER.BASE_LR
        )
        print("============ using the AdamW optimizer for all the parameters ============")
        optimizer = maybe_add_gradient_clipping(cfg, optimizer)
    else: # using adamw by default
        optimizer = torch.optim.AdamW(
            params, cfg.SOLVER.BASE_LR
        )
        print("============ using the AdamW optimizer for all the parameters ============")
